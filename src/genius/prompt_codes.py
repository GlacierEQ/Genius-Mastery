...
_CODES = (
    ...
    PromptCode("VERBATIM-FIRST", "quality", "When prior operator language is material, recover and preserve exact source-bearing wording before relying on summaries or reinterpretation."),
    ...
    PromptCode("LONG-RUN", "execution", "Continue through the strongest coherent tranche of executable work; do not stop after diagnosis, a token patch, or a short summary while authorized high-value work remains."),
    ...
    PromptCode("MISSION-DELTA", "modifier", "Count a run as progress only when it directly changes at least one operator objective state and records the before/after delta."),
    PromptCode("DIVERSE OUTPUTS", "modifier", "Prefer heterogeneous durable outputs when distinct artifacts, tests, records, code, or evidence surfaces create additional value."),
